...
refreshResults <- function(inp,session){
  updateSelectInput(session,"resModLst",choices=list.dirs("./analysis",recursive=FALSE))
  if(inp$resModLst!="" && !is.null(inp$resModLst))  updateSelectInput(session,"resModAll",choices=list.files(inp$resModLst,pattern=ifelse(inp$typeRes=="PDF","\\.pdf$","\\.html$")))
}
#------------------------------------------ udpateResultList ------------------------------------------
#' @export
# Function to update the list with result files (in case new model is selected)
udpateResultList <- function(inp,session){
  if(inp$resModLst!="" && !is.null(inp$resModLst)) updateSelectInput(session,"resModAll",choices=list.files(inp$resModLst,pattern=ifelse(inp$typeRes=="PDF","\\.pdf$","\\.html$")))
}
#------------------------------------------ changeResults ------------------------------------------
#' @export
# Function to change the list with result files (in case different extension is selected)
changeResults <- function(inp,session){
  if(inp$resModLst!="" && !is.null(inp$resModLst)) updateSelectInput(session,"resModAll",choices=list.files(inp$resModLst,pattern=ifelse(inp$typeRes=="PDF","\\.pdf$","\\.html$")))
}
#------------------------------------------ showResults ------------------------------------------
#' @export
...
showResults <- function(inp,session){
  ...
  if(toupper(inp$nameRes)%in%toupper(gsub("\\.pdf|\\.html","",inp$resModAll))){
    ...
    if(inp$typeRes=="PDF"){
      R3port::ltx_combine(list(paste0(inp$resModLst,"/",sub("\\.pdf$",".tex.rawtex",inp$resModAll))),
                          out=paste0(inp$resModLst,"/",inp$nameRes,".tex"),show=TRUE)
    }else if(inp$typeRes=="HTML"){
      R3port::html_combine(list(paste0(inp$resModLst,"/",sub("\\.html$",".html.rawhtml",inp$resModAll))),
                           out=paste0(inp$resModLst,"/",inp$nameRes,".html"),show=TRUE,
