...
module_scripts_server <- function(id,files=NULL,scripts=NULL,loc="temp") {
  ...
  moduleServer(id,
    function(input, output, session) {
      ...
      # Reactive poll to follow the run progress of the script (used this method because name of script output is variable!)
      monout <- reactivePoll(500, session,
        checkFunc = function(){
          if(file.exists(paste0(normalizePath(loc),"/scriptres",uid(),".out"))) file.mtime(paste0(normalizePath(loc),"/scriptres",uid(),".out")) else ""
        },
      ...
      observe({
        monout()
        isolate({
          if(file.exists(paste0(normalizePath(loc),"/scriptres",uid(),".out"))) fr <- paste(readLines(paste0(normalizePath(loc),"/scriptres",uid(),".out")),collapse="\n") else fr <- ""
          output$scriptprogress <- renderText(fr)  
        })
      })
    }
  )
